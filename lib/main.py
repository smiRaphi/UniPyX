                ...
                if os.path.exists(log):
                    ...
                    for x in msplit(lg.split('\n')[0].split(' - ',1)[1],[' - [ ',' ] [ ',' ] - ',' [ ',' ] ',' stub : ',' Ovl like : ',' - ',' , ']):
                        if x == '( RESOURCES ONLY ! no CODE )': ts.append('Resources Only')
                            ...
                            x = x.strip(' ,!:;-()[]')
                            if x and x.lower() not in ('genuine','unknown','more than necessary','sections') and not x.lower().endswith(' sections') and not (len(x) in (2,4) and x.isdigit()) and not (len(x) in (8,9) and x.replace('-','').isdigit()) and\
