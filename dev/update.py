...
def supdate(c:Cache,k:str,inf:dict):
    ...
    for f in inf['fs']:
        ...
        if u == '.': tts = -1
            ...
            if repo not in ('VirusTotal/yara','allcoolthingsatoneplace/UnrealPakTool','joncampbell123/dosbox-x','GotthardtZ/paq8gen','skandau/paq8sk','kaitz/paq8pxv','kaitz/fxcm','schnaader/precomp-cpp','graalvm/graalvm-ce-builds'):
...
if __name__ == '__main__':
    ...
    if len(argv) in (2,3) and argv[1] == 't':
        if len(argv) == 2: print(t())
            ...
            elif ((len(argv[2]) == 25 and argv[2][22] == ':') or len(argv[2]) == 24) and argv[2][4] == argv[2][7] == '-' and argv[2][10] == 'T' and argv[2][13] == argv[2][16] == ':' and argv[2][19] in '+-': print(ft(argv[2],'%Y-%m-%dT%H:%M:%S%z'))
